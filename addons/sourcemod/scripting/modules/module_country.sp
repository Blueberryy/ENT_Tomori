...
ConVar gH_Cvar_Tomori_CountryFilter_Block_Connect;				//Enabled or disable CountryFilter for joining to server (0 - Disable, 1 - Enabled)
ConVar gH_Cvar_Tomori_CountryFilter_Block_CT;					//Enabled or disable CountryFilter for joining ct team (0 - Disable, 1 - Enabled)
ConVar gH_Cvar_Tomori_CountryFilter_BlockMode;					//Filter ListMode (0 - Whitelist, 1 - Blacklist)
ConVar gH_Cvar_Tomori_CountryFilter_List;						//List (Used like BlockMode set)
...
public bool OnClientConnect(int client, char[] rejectmsg, int maxlen)
{
	if (gH_Cvar_Tomori_CountryFilter_Enabled.BoolValue)
	{
		...
		if (gH_Cvar_Tomori_CountryFilter_Block_Connect.BoolValue)
		{
			if (RejectJoin(code2))
			{
				CreateTimer(0.1, cfTimer, client);
				Format(rejectmsg, sizeof(maxlen), "%t", "Tomori Country Join Blocked", country);
				//return false;
			}
		}
	}
...
{
	char rejectmsg[255];
	Format(rejectmsg, sizeof(rejectmsg), "%t", "Tomori Country Join Blocked", country);
