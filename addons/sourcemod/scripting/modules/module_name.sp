...
ConVar gH_Cvar_Tomori_Name_ExtraChars;				//Add extra characters to allow in name
ConVar gH_Cvar_Tomori_Name_Badname_AutoChange;		//AutoChange illegal names
ConVar gH_Cvar_Tomori_Name_Badname_AutoChangeTo;	//Rename to this name + random number

ConVar gH_Cvar_Tomori_Name_MaxChanges;				//Punish After x name change in a map
ConVar gH_Cvar_Tomori_Name_MaxChanges_Punishment;	//Punishment type
ConVar gH_Cvar_Tomori_Name_MaxChanges_Bantime;		//Bantime (if type is ban)
ConVar gH_Cvar_Tomori_Name_MaxChanges_Reason;		//Ban/Kick reason for bad name

ConVar gH_Cvar_Tomori_Name_Website_Filter;			//Filter websites from names
ConVar gH_Cvar_Tomori_Name_IP_Filter;				//Filter IPs from names
ConVar gH_Cvar_Tomori_Name_Badword_Filter;			//Filter badwords from names

int gShadow_Client_ChangedTime[MAXPLAYERS+1] = 0;
bool gShadow_Client_ChangedByTomori[MAXPLAYERS+1] = false;
bool gShadow_Client_ChangedOnJoin[MAXPLAYERS+1] = false;
		...
		if (IsValidClient(idx))
		{
			...
			if (Filters_Loaded)
			{
				...
				if (ReadCompleted && gH_Cvar_Tomori_Name_Badword_Filter.BoolValue)
				{
					...
					for (int i = 0; i < 128; i++)
					{
						Format(Search, 2, name[i]);
						if (!StrEqual(Search, "") && StrContains(name, Search, false) != -1)
						{
							ReplaceString(name, MAX_NAME_LENGTH, Search, "");
						}
					}
					
					for (int i = 0; i <= WordLines; i++)
					{
						if (!StrEqual(WordCount[i], "") && StrContains(name, WordCount[i], false) != -1)
						{
							return true;
						}
					}
		...
		if (gH_Cvar_Tomori_Name_Enabled.BoolValue && !gShadow_Client_ChangedByTomori[client])
		{
			...
			for (int idx = 1; idx <= MaxClients; idx++)
			{
				if (IsValidClient(idx))
				{
					...
					if (StrEqual(tempname, playerName))
					{
			...
			if (CheckNameAll(playerName))
			{
				...
				Format(gShadow_Client_NameToChange[client], sizeof(gShadow_Client_NameToChange), oldname);
			...
			if ((gShadow_Client_ChangedTime[client] != gH_Cvar_Tomori_Name_MaxChanges.IntValue) && ((gH_Cvar_Tomori_Name_MaxChanges.IntValue - (gShadow_Client_ChangedTime[client] + 1)) != 0))
			{
				...
				Format(OldName, sizeof(OldName), "\x07%s\x0B", TempName);
				GetEventString(event, "newname", playerName, sizeof(playerName));
				Format(NewName, sizeof(NewName), "\x07%s\x0B", playerName);
			...
			else if (gShadow_Client_ChangedTime[client] == gH_Cvar_Tomori_Name_MaxChanges.IntValue)
			{
				...
				Format(Reason, sizeof(Reason), "%s %s", "[Tomori]", tempstring);
...
{
	ReplaceString(checkit, MAX_NAME_LENGTH, " ", "");
...
stock void ENT_RenameIllegal(int client, const char[] DefaultName = "Changed")
{
	...
	Format(NewNameToGo, sizeof(NewNameToGo), ENT_GenerateName(DefaultName, 0, 999999));
	
	char CheckName[MAX_NAME_LENGTH];
	
	for (int idx = 1; idx <= MaxClients; idx++)
	{
		if (IsValidClient(idx) && (idx != client))
		{
			GetClientName(client, CheckName, sizeof(CheckName));
			if (StrEqual(CheckName, NewNameToGo))
				Format(NewNameToGo, sizeof(NewNameToGo), ENT_GenerateName(DefaultName, 0, 999999));
		}
	}
...
stock char ENT_GenerateName(const char[] Main = "Changed", int min, int max)
{
	...
	Format(RandomName, sizeof(RandomName), "%s%s", Main, RandomIntToName);
	return RandomName;
}