...
ConVar gH_Cvar_Tomori_Profile_ApiKey;					//Api key to get Data from steam

ConVar gH_Cvar_Tomori_Profile_MinHours;					//Minimum hour to play to join
ConVar gH_Cvar_Tomori_Profile_MinLevel;					//Minimum steam level to join

ConVar gH_Cvar_Tomori_Profile_IgnoreAdmins;				//Disable admin check on joining
ConVar gH_Cvar_Tomori_Profile_IgnoreFlag;				//Admin flag to ignore
ConVar gH_Cvar_Tomori_Profile_OnlyPrime;				//Minimum steam level to play in server
ConVar gH_Cvar_Tomori_Profile_MinHours_NP;				//Minimum hour to play to join for non primes
ConVar gH_Cvar_Tomori_Profile_MinLevel_NP;				//Minimum steam level to play to join for non primes
...
public void Profile_OnClientPostAdminCheck(int client)
{
	...
	GetFlagInt(ignoreflag);
	int ign_flag = StringToInt(ignoreflag);
	
	if (Client_HasAdminFlags(client, ign_flag))
		gShadow_Tomori_UserAdmin[client] = true;
	
	if (gH_Cvar_Tomori_Profile_Enabled.BoolValue) RequestHours(client, auth);
}
...
{
	char request_url[512];
	Format(request_url, sizeof(request_url), "https://api.steampowered.com/IPlayerService/GetSteamLevel/v1/?key=%s&format=json&input_json={\"steamid\":%s}", cAPIKey, auth);
	Handle request = SteamWorks_CreateHTTPRequest(k_EHTTPMethodGET, request_url);
